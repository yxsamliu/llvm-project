llvm=/home/yaxunl/git/llvm2
#cd /c/git/llvm2
cd $llvm

ninja -C assert install || exit -1

PATH=$llvm/assert/install/bin:$PATH
...
clang++ -mllvm -debug-only=pgo-instrumentation -Wno-unused-value -O2 -fprofile-generate=my --offload-arch=$gfx -save-temps -x hip tmp_rovodev_hip_pgo_comprehensive_test.hip -o tmp_rovodev_hip_pgo_test_direct

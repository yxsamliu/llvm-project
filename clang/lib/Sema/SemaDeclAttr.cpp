...
ProcessDeclAttribute(Sema &S, Scope *scope, Decl *D, const ParsedAttr &AL,
  ...
  case ParsedAttr::AT_AMDGPUSpatialClusterKernel:
    ...
    break;
  case ParsedAttr::AT_AMDGPUAsymmetricClusterClampKernel:
    S.AMDGPU().handleAMDGPUAsymmetricClusterClampKernelAttr(D, AL);
    break;
  case ParsedAttr::AT_AMDGPUNoRankSpecialization:
    S.AMDGPU().handleAMDGPUNoRankSpecializationAttr(D, AL);
    break;
  case ParsedAttr::AT_AMDGPUEnableWasp:
    S.AMDGPU().handleAMDGPUEnableWaspAttr(D, AL);
    break;
  ...
  if (!isKernelDecl(D)) {
    if (const auto *A = D->getAttr<AMDGPUFlatWorkGroupSizeAttr>()) {
      ...
      Diag(D->getLocation(), diag::err_attribute_wrong_decl_type)
          << A << A->isRegularKeywordAttribute() << ExpectedKernelFunction;
      D->setInvalidDecl();
    } else if (const auto *A = D->getAttr<AMDGPUWavegroupKernelAttr>()) {
      Diag(D->getLocation(), diag::err_attribute_wrong_decl_type)
          << A << A->isRegularKeywordAttribute() << ExpectedKernelFunction;
      D->setInvalidDecl();
    }

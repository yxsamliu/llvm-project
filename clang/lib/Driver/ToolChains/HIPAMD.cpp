...
HIPAMDToolChain::TranslateArgs(const llvm::opt::DerivedArgList &Args,
  ...
  for (Arg *A : Args) {
    bool IsProf = false;
    for (auto O: {options::OPT_fprofile_generate_EQ, options::OPT_fprofile_use_EQ}){
      if (A->getOption().matches(O)) {
        StringRef ProfileFile = A->getValue();
        std::string DeviceProfileFile = std::string(ProfileFile);
        DeviceProfileFile = DeviceProfileFile + ".amdgcn-amd-amdhsa";
        DAL->AddJoinedArg(A, Opts.getOption(O), DeviceProfileFile);
        A->claim();
        IsProf = true;
      }
    }

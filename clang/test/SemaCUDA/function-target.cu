// RUN: %clang_cc1 -fsyntax-only -verify=expected,com %s
// RUN: %clang_cc1 -fsyntax-only -fcuda-is-device -verify=dev,com %s
...
__host__ void d1h(void); // dev-note {{candidate function not viable: call to __host__ function from __device__ function}}
__device__ void d1d(void);
__host__ __device__ void d1hd(void);
__global__ void d1g(void); // com-note {{'d1g' declared here}}

__device__ void d1(void) {
  d1h(); // dev-error {{no matching function}}
  d1d();
  d1hd();
  d1g<<<1, 1>>>(); // com-error {{reference to __global__ function 'd1g' in __device__ function}}
}

; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py UTC_ARGS: --version 5
; RUN: llc -mtriple=amdgcn -mcpu=verde < %s | FileCheck -check-prefixes=GCN,SI %s
; RUN: llc -mtriple=amdgcn -mcpu=tonga < %s | FileCheck -check-prefixes=GCN,VI %s
; RUN: llc -mtriple=amdgcn -mcpu=gfx1300 -mattr=+real-true16 < %s | FileCheck -check-prefixes=GFX13,GFX13-TRUE16 %s
; RUN: llc -mtriple=amdgcn -mcpu=gfx1300 -mattr=-real-true16 < %s | FileCheck -check-prefixes=GFX13,GFX13-FAKE16 %s

define amdgpu_kernel void @truncstore_arg_v16i32_to_v16i8(ptr addrspace(1) %out, <16 x i32> %in) {
; SI-LABEL: truncstore_arg_v16i32_to_v16i8:
; SI:       ; %bb.0:
; SI-NEXT:    s_load_dwordx16 s[8:23], s[4:5], 0x19
; SI-NEXT:    s_load_dwordx2 s[0:1], s[4:5], 0x9
; SI-NEXT:    s_mov_b32 s3, 0xf000
; SI-NEXT:    s_mov_b32 s2, -1
; SI-NEXT:    s_waitcnt lgkmcnt(0)
; SI-NEXT:    s_and_b32 s5, s22, 0xff
; SI-NEXT:    s_lshl_b32 s4, s23, 24
; SI-NEXT:    s_lshl_b32 s5, s5, 16
; SI-NEXT:    s_or_b32 s4, s4, s5
; SI-NEXT:    s_lshl_b32 s5, s21, 8
; SI-NEXT:    s_and_b32 s6, s20, 0xff
; SI-NEXT:    s_or_b32 s5, s6, s5
; SI-NEXT:    s_and_b32 s5, s5, 0xffff
; SI-NEXT:    s_and_b32 s6, s18, 0xff
; SI-NEXT:    s_or_b32 s4, s5, s4
; SI-NEXT:    s_lshl_b32 s5, s19, 24
; SI-NEXT:    s_lshl_b32 s6, s6, 16
; SI-NEXT:    s_or_b32 s5, s5, s6
; SI-NEXT:    s_lshl_b32 s6, s17, 8
; SI-NEXT:    s_and_b32 s7, s16, 0xff
; SI-NEXT:    s_or_b32 s6, s7, s6
; SI-NEXT:    s_and_b32 s6, s6, 0xffff
; SI-NEXT:    s_and_b32 s7, s14, 0xff
; SI-NEXT:    s_or_b32 s5, s6, s5
; SI-NEXT:    s_lshl_b32 s6, s15, 24
; SI-NEXT:    s_lshl_b32 s7, s7, 16
; SI-NEXT:    s_or_b32 s6, s6, s7
; SI-NEXT:    s_lshl_b32 s7, s13, 8
; SI-NEXT:    s_and_b32 s12, s12, 0xff
; SI-NEXT:    s_or_b32 s7, s12, s7
; SI-NEXT:    s_and_b32 s7, s7, 0xffff
; SI-NEXT:    s_and_b32 s10, s10, 0xff
; SI-NEXT:    s_lshl_b32 s9, s9, 8
; SI-NEXT:    s_and_b32 s8, s8, 0xff
; SI-NEXT:    s_or_b32 s6, s7, s6
; SI-NEXT:    s_lshl_b32 s7, s11, 24
; SI-NEXT:    s_lshl_b32 s10, s10, 16
; SI-NEXT:    s_or_b32 s8, s8, s9
; SI-NEXT:    s_or_b32 s7, s7, s10
; SI-NEXT:    s_and_b32 s8, s8, 0xffff
; SI-NEXT:    s_or_b32 s7, s8, s7
; SI-NEXT:    v_mov_b32_e32 v0, s7
; SI-NEXT:    v_mov_b32_e32 v1, s6
; SI-NEXT:    v_mov_b32_e32 v2, s5
; SI-NEXT:    v_mov_b32_e32 v3, s4
; SI-NEXT:    buffer_store_dwordx4 v[0:3], off, s[0:3], 0
; SI-NEXT:    s_endpgm
;
; VI-LABEL: truncstore_arg_v16i32_to_v16i8:
; VI:       ; %bb.0:
; VI-NEXT:    s_load_dwordx16 s[8:23], s[4:5], 0x64
; VI-NEXT:    v_mov_b32_e32 v0, 0xc0c0004
; VI-NEXT:    s_load_dwordx2 s[0:1], s[4:5], 0x24
; VI-NEXT:    s_waitcnt lgkmcnt(0)
; VI-NEXT:    v_mov_b32_e32 v2, s23
; VI-NEXT:    v_mov_b32_e32 v1, s21
; VI-NEXT:    v_perm_b32 v2, s22, v2, v0
; VI-NEXT:    v_perm_b32 v1, s20, v1, v0
; VI-NEXT:    v_lshlrev_b32_e32 v2, 16, v2
; VI-NEXT:    v_or_b32_e32 v3, v1, v2
; VI-NEXT:    v_mov_b32_e32 v2, s19
; VI-NEXT:    v_mov_b32_e32 v1, s17
; VI-NEXT:    v_perm_b32 v2, s18, v2, v0
; VI-NEXT:    v_perm_b32 v1, s16, v1, v0
; VI-NEXT:    v_lshlrev_b32_e32 v2, 16, v2
; VI-NEXT:    v_mov_b32_e32 v4, s15
; VI-NEXT:    v_or_b32_e32 v2, v1, v2
; VI-NEXT:    v_mov_b32_e32 v1, s13
; VI-NEXT:    v_perm_b32 v4, s14, v4, v0
; VI-NEXT:    v_perm_b32 v1, s12, v1, v0
; VI-NEXT:    v_lshlrev_b32_e32 v4, 16, v4
; VI-NEXT:    v_or_b32_e32 v1, v1, v4
; VI-NEXT:    v_mov_b32_e32 v4, s9
; VI-NEXT:    v_mov_b32_e32 v5, s11
; VI-NEXT:    v_perm_b32 v4, s8, v4, v0
; VI-NEXT:    v_perm_b32 v0, s10, v5, v0
; VI-NEXT:    v_lshlrev_b32_e32 v0, 16, v0
; VI-NEXT:    v_or_b32_e32 v0, v4, v0
; VI-NEXT:    v_mov_b32_e32 v5, s1
; VI-NEXT:    v_mov_b32_e32 v4, s0
; VI-NEXT:    flat_store_dwordx4 v[4:5], v[0:3]
; VI-NEXT:    s_endpgm
;
; GFX13-LABEL: truncstore_arg_v16i32_to_v16i8:
; GFX13:       ; %bb.0:
; GFX13-NEXT:    s_clause 0x1
; GFX13-NEXT:    s_load_b512 s[8:23], s[4:5], 0x64
; GFX13-NEXT:    s_load_b64 s[0:1], s[4:5], 0x24
; GFX13-NEXT:    v_mov_b32_e32 v0, 0xc0c0004
; GFX13-NEXT:    s_wait_kmcnt 0x0
; GFX13-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX13-NEXT:    v_perm_b32 v1, s20, s21, v0
; GFX13-NEXT:    s_lshl_b32 s2, s23, 8
; GFX13-NEXT:    s_and_b32 s3, s22, 0xff
; GFX13-NEXT:    v_perm_b32 v2, s16, s17, v0
; GFX13-NEXT:    s_lshl_b32 s4, s19, 8
; GFX13-NEXT:    s_and_b32 s5, s18, 0xff
; GFX13-NEXT:    v_perm_b32 v5, s12, s13, v0
; GFX13-NEXT:    s_lshl_b32 s6, s15, 8
; GFX13-NEXT:    s_and_b32 s7, s14, 0xff
; GFX13-NEXT:    v_perm_b32 v0, s8, s9, v0
; GFX13-NEXT:    s_lshl_b32 s8, s11, 8
; GFX13-NEXT:    s_and_b32 s9, s10, 0xff
; GFX13-NEXT:    s_or_b32 s2, s3, s2
; GFX13-NEXT:    s_or_b32 s3, s5, s4
; GFX13-NEXT:    s_or_b32 s4, s7, s6
; GFX13-NEXT:    s_or_b32 s5, s9, s8
; GFX13-NEXT:    s_lshl_b32 s2, s2, 16
; GFX13-NEXT:    s_lshl_b32 s3, s3, 16
; GFX13-NEXT:    s_lshl_b32 s4, s4, 16
; GFX13-NEXT:    s_lshl_b32 s5, s5, 16
; GFX13-NEXT:    v_dual_mov_b32 v4, 0 :: v_dual_bitop2_b32 v3, s2, v1 bitop3:0x54
; GFX13-NEXT:    v_or_b32_e32 v2, s3, v2
; GFX13-NEXT:    v_or_b32_e32 v1, s4, v5
; GFX13-NEXT:    v_or_b32_e32 v0, s5, v0
; GFX13-NEXT:    global_store_b128 v4, v[0:3], s[0:1]
; GFX13-NEXT:    s_endpgm
  %trunc = trunc <16 x i32> %in to <16 x i8>
  store <16 x i8> %trunc, ptr addrspace(1) %out
  ret void
}

define amdgpu_kernel void @truncstore_arg_v16i64_to_v16i8(ptr addrspace(1) %out, <16 x i64> %in) {
; SI-LABEL: truncstore_arg_v16i64_to_v16i8:
; SI:       ; %bb.0:
; SI-NEXT:    s_load_dwordx16 s[16:31], s[4:5], 0x39
; SI-NEXT:    s_load_dwordx2 s[36:37], s[4:5], 0x9
; SI-NEXT:    s_load_dwordx16 s[0:15], s[4:5], 0x29
; SI-NEXT:    s_mov_b32 s39, 0xf000
; SI-NEXT:    s_mov_b32 s38, -1
; SI-NEXT:    s_waitcnt lgkmcnt(0)
; SI-NEXT:    s_and_b32 s3, s28, 0xff
; SI-NEXT:    s_lshl_b32 s1, s30, 24
; SI-NEXT:    s_lshl_b32 s3, s3, 16
; SI-NEXT:    s_or_b32 s1, s1, s3
; SI-NEXT:    s_lshl_b32 s3, s26, 8
; SI-NEXT:    s_and_b32 s5, s24, 0xff
; SI-NEXT:    s_or_b32 s3, s5, s3
; SI-NEXT:    s_and_b32 s3, s3, 0xffff
; SI-NEXT:    s_and_b32 s5, s20, 0xff
; SI-NEXT:    s_or_b32 s1, s3, s1
; SI-NEXT:    s_lshl_b32 s3, s22, 24
; SI-NEXT:    s_lshl_b32 s5, s5, 16
; SI-NEXT:    s_or_b32 s3, s3, s5
; SI-NEXT:    s_lshl_b32 s5, s18, 8
; SI-NEXT:    s_and_b32 s7, s16, 0xff
; SI-NEXT:    s_or_b32 s5, s7, s5
; SI-NEXT:    s_and_b32 s5, s5, 0xffff
; SI-NEXT:    s_and_b32 s7, s12, 0xff
; SI-NEXT:    s_or_b32 s3, s5, s3
; SI-NEXT:    s_lshl_b32 s5, s14, 24
; SI-NEXT:    s_lshl_b32 s7, s7, 16
; SI-NEXT:    s_or_b32 s5, s5, s7
; SI-NEXT:    s_lshl_b32 s7, s10, 8
; SI-NEXT:    s_and_b32 s8, s8, 0xff
; SI-NEXT:    s_and_b32 s4, s4, 0xff
; SI-NEXT:    s_lshl_b32 s2, s2, 8
; SI-NEXT:    s_and_b32 s0, s0, 0xff
; SI-NEXT:    s_or_b32 s7, s8, s7
; SI-NEXT:    s_lshl_b32 s6, s6, 24
; SI-NEXT:    s_lshl_b32 s4, s4, 16
; SI-NEXT:    s_or_b32 s0, s0, s2
; SI-NEXT:    s_and_b32 s7, s7, 0xffff
; SI-NEXT:    s_or_b32 s4, s6, s4
; SI-NEXT:    s_and_b32 s0, s0, 0xffff
; SI-NEXT:    s_or_b32 s5, s7, s5
; SI-NEXT:    s_or_b32 s0, s0, s4
; SI-NEXT:    v_mov_b32_e32 v0, s0
; SI-NEXT:    v_mov_b32_e32 v1, s5
; SI-NEXT:    v_mov_b32_e32 v2, s3
; SI-NEXT:    v_mov_b32_e32 v3, s1
; SI-NEXT:    buffer_store_dwordx4 v[0:3], off, s[36:39], 0
; SI-NEXT:    s_endpgm
;
; VI-LABEL: truncstore_arg_v16i64_to_v16i8:
; VI:       ; %bb.0:
; VI-NEXT:    s_load_dwordx16 s[16:31], s[4:5], 0xe4
; VI-NEXT:    v_mov_b32_e32 v0, 0xc0c0004
; VI-NEXT:    s_load_dwordx2 s[34:35], s[4:5], 0x24
; VI-NEXT:    s_load_dwordx16 s[0:15], s[4:5], 0xa4
; VI-NEXT:    s_waitcnt lgkmcnt(0)
; VI-NEXT:    v_mov_b32_e32 v2, s30
; VI-NEXT:    v_mov_b32_e32 v1, s26
; VI-NEXT:    v_perm_b32 v2, s28, v2, v0
; VI-NEXT:    v_perm_b32 v1, s24, v1, v0
; VI-NEXT:    v_lshlrev_b32_e32 v2, 16, v2
; VI-NEXT:    v_or_b32_e32 v3, v1, v2
; VI-NEXT:    v_mov_b32_e32 v2, s22
; VI-NEXT:    v_mov_b32_e32 v1, s18
; VI-NEXT:    v_perm_b32 v2, s20, v2, v0
; VI-NEXT:    v_perm_b32 v1, s16, v1, v0
; VI-NEXT:    v_lshlrev_b32_e32 v2, 16, v2
; VI-NEXT:    v_mov_b32_e32 v4, s14
; VI-NEXT:    v_or_b32_e32 v2, v1, v2
; VI-NEXT:    v_mov_b32_e32 v1, s10
; VI-NEXT:    v_perm_b32 v4, s12, v4, v0
; VI-NEXT:    v_perm_b32 v1, s8, v1, v0
; VI-NEXT:    v_lshlrev_b32_e32 v4, 16, v4
; VI-NEXT:    v_or_b32_e32 v1, v1, v4
; VI-NEXT:    v_mov_b32_e32 v4, s2
; VI-NEXT:    v_mov_b32_e32 v5, s6
; VI-NEXT:    v_perm_b32 v4, s0, v4, v0
; VI-NEXT:    v_perm_b32 v0, s4, v5, v0
; VI-NEXT:    v_lshlrev_b32_e32 v0, 16, v0
; VI-NEXT:    v_or_b32_e32 v0, v4, v0
; VI-NEXT:    v_mov_b32_e32 v4, s34
; VI-NEXT:    v_mov_b32_e32 v5, s35
; VI-NEXT:    flat_store_dwordx4 v[4:5], v[0:3]
; VI-NEXT:    s_endpgm
;
; GFX13-LABEL: truncstore_arg_v16i64_to_v16i8:
; GFX13:       ; %bb.0:
; GFX13-NEXT:    s_clause 0x2
; GFX13-NEXT:    s_load_b512 s[36:51], s[4:5], 0xe4
; GFX13-NEXT:    s_load_b512 s[8:23], s[4:5], 0xa4
; GFX13-NEXT:    s_load_b64 s[0:1], s[4:5], 0x24
; GFX13-NEXT:    v_mov_b32_e32 v0, 0xc0c0004
; GFX13-NEXT:    s_wait_kmcnt 0x0
; GFX13-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX13-NEXT:    v_perm_b32 v1, s44, s46, v0
; GFX13-NEXT:    s_lshl_b32 s2, s50, 8
; GFX13-NEXT:    s_and_b32 s3, s48, 0xff
; GFX13-NEXT:    v_perm_b32 v2, s36, s38, v0
; GFX13-NEXT:    s_lshl_b32 s4, s42, 8
; GFX13-NEXT:    s_and_b32 s5, s40, 0xff
; GFX13-NEXT:    v_perm_b32 v5, s16, s18, v0
; GFX13-NEXT:    s_lshl_b32 s6, s22, 8
; GFX13-NEXT:    s_and_b32 s7, s20, 0xff
; GFX13-NEXT:    v_perm_b32 v0, s8, s10, v0
; GFX13-NEXT:    s_lshl_b32 s8, s14, 8
; GFX13-NEXT:    s_and_b32 s9, s12, 0xff
; GFX13-NEXT:    s_or_b32 s2, s3, s2
; GFX13-NEXT:    s_or_b32 s3, s5, s4
; GFX13-NEXT:    s_or_b32 s4, s7, s6
; GFX13-NEXT:    s_or_b32 s5, s9, s8
; GFX13-NEXT:    s_lshl_b32 s2, s2, 16
; GFX13-NEXT:    s_lshl_b32 s3, s3, 16
; GFX13-NEXT:    s_lshl_b32 s4, s4, 16
; GFX13-NEXT:    s_lshl_b32 s5, s5, 16
; GFX13-NEXT:    v_dual_mov_b32 v4, 0 :: v_dual_bitop2_b32 v3, s2, v1 bitop3:0x54
; GFX13-NEXT:    v_or_b32_e32 v2, s3, v2
; GFX13-NEXT:    v_or_b32_e32 v1, s4, v5
; GFX13-NEXT:    v_or_b32_e32 v0, s5, v0
; GFX13-NEXT:    global_store_b128 v4, v[0:3], s[0:1]
; GFX13-NEXT:    s_endpgm
  %trunc = trunc <16 x i64> %in to <16 x i8>
  store <16 x i8> %trunc, ptr addrspace(1) %out
  ret void
}

define void @truncstore_v5i32_to_v5i1(ptr addrspace(1) %out, <5 x i32> %val) {
; SI-LABEL: truncstore_v5i32_to_v5i1:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_and_b32_e32 v3, 1, v3
; SI-NEXT:    v_and_b32_e32 v2, 1, v2
; SI-NEXT:    v_lshlrev_b32_e32 v3, 1, v3
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    v_and_b32_e32 v3, 1, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 2, v3
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    v_and_b32_e32 v3, 1, v5
; SI-NEXT:    v_lshlrev_b32_e32 v3, 3, v3
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    v_lshlrev_b32_e32 v3, 4, v6
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_and_b32_e32 v2, 31, v2
; SI-NEXT:    buffer_store_byte v2, v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: truncstore_v5i32_to_v5i1:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    v_and_b32_e32 v3, 1, v3
; VI-NEXT:    v_and_b32_e32 v2, 1, v2
; VI-NEXT:    v_lshlrev_b16_e32 v3, 1, v3
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_and_b32_e32 v3, 1, v4
; VI-NEXT:    v_lshlrev_b16_e32 v3, 2, v3
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_and_b32_e32 v3, 1, v5
; VI-NEXT:    v_lshlrev_b16_e32 v3, 3, v3
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_lshlrev_b16_e32 v3, 4, v6
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_and_b32_e32 v2, 31, v2
; VI-NEXT:    flat_store_byte v[0:1], v2
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-TRUE16-LABEL: truncstore_v5i32_to_v5i1:
; GFX13-TRUE16:       ; %bb.0:
; GFX13-TRUE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_expcnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-TRUE16-NEXT:    v_and_b16 v2.h, v3.l, 1
; GFX13-TRUE16-NEXT:    v_and_b16 v3.l, v4.l, 1
; GFX13-TRUE16-NEXT:    v_and_b16 v3.h, v5.l, 1
; GFX13-TRUE16-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v2.h, 1, v2.h
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v3.l, 2, v3.l
; GFX13-TRUE16-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v3.h, 3, v3.h
; GFX13-TRUE16-NEXT:    v_bitop3_b16 v2.l, v2.l, v2.h, 1 bitop3:0xec
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v2.h, 4, v6.l
; GFX13-TRUE16-NEXT:    s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
; GFX13-TRUE16-NEXT:    v_bitop3_b16 v2.l, v2.l, v3.h, v3.l bitop3:0xfe
; GFX13-TRUE16-NEXT:    v_bitop3_b16 v2.l, v2.l, 31, v2.h bitop3:0xc8
; GFX13-TRUE16-NEXT:    global_store_b8 v[0:1], v2, off
; GFX13-TRUE16-NEXT:    s_set_pc_i64 s[30:31]
;
; GFX13-FAKE16-LABEL: truncstore_v5i32_to_v5i1:
; GFX13-FAKE16:       ; %bb.0:
; GFX13-FAKE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_expcnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v3, 1, v3
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v4, 1, v4
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v5, 1, v5
; GFX13-FAKE16-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v3, 1, v3
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v4, 2, v4
; GFX13-FAKE16-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(NEXT) | instid1(VALU_DEP_3)
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v5, 3, v5
; GFX13-FAKE16-NEXT:    v_bitop3_b16 v2, v2, v3, 1 bitop3:0xec
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v3, 4, v6
; GFX13-FAKE16-NEXT:    s_delay_alu instid0(VALU_DEP_2) | instskip(NEXT) | instid1(VALU_DEP_1)
; GFX13-FAKE16-NEXT:    v_bitop3_b16 v2, v2, v5, v4 bitop3:0xfe
; GFX13-FAKE16-NEXT:    v_bitop3_b16 v2, v2, 31, v3 bitop3:0xc8
; GFX13-FAKE16-NEXT:    global_store_b8 v[0:1], v2, off
; GFX13-FAKE16-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = trunc <5 x i32> %val to <5 x i1>
  store <5 x i1> %trunc, ptr addrspace(1) %out
  ret void
}

define void @truncstore_v5i32_to_v5i8(ptr addrspace(1) %out, <5 x i32> %val) {
; SI-LABEL: truncstore_v5i32_to_v5i8:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_and_b32_e32 v4, 0xff, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 8, v3
; SI-NEXT:    v_and_b32_e32 v2, 0xff, v2
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_lshlrev_b32_e32 v5, 24, v5
; SI-NEXT:    v_lshlrev_b32_e32 v4, 16, v4
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_or_b32_e32 v4, v5, v4
; SI-NEXT:    v_and_b32_e32 v2, 0xffff, v2
; SI-NEXT:    v_or_b32_e32 v2, v2, v4
; SI-NEXT:    buffer_store_byte v6, v[0:1], s[4:7], 0 addr64 offset:4
; SI-NEXT:    buffer_store_dword v2, v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: truncstore_v5i32_to_v5i8:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    s_mov_b32 s4, 0xc0c0004
; VI-NEXT:    v_perm_b32 v2, v2, v3, s4
; VI-NEXT:    v_perm_b32 v3, v4, v5, s4
; VI-NEXT:    v_lshlrev_b32_e32 v3, 16, v3
; VI-NEXT:    v_or_b32_e32 v4, v2, v3
; VI-NEXT:    v_add_u32_e32 v2, vcc, 4, v0
; VI-NEXT:    v_addc_u32_e32 v3, vcc, 0, v1, vcc
; VI-NEXT:    flat_store_byte v[2:3], v6
; VI-NEXT:    flat_store_dword v[0:1], v4
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-LABEL: truncstore_v5i32_to_v5i8:
; GFX13:       ; %bb.0:
; GFX13-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-NEXT:    s_wait_expcnt 0x0
; GFX13-NEXT:    s_wait_samplecnt 0x0
; GFX13-NEXT:    s_wait_rtscnt 0x0
; GFX13-NEXT:    s_wait_kmcnt 0x0
; GFX13-NEXT:    v_perm_b32 v2, v2, v3, 0xc0c0004
; GFX13-NEXT:    v_perm_b32 v3, v4, v5, 0xc0c0004
; GFX13-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX13-NEXT:    v_lshl_or_b32 v2, v3, 16, v2
; GFX13-NEXT:    s_clause 0x1
; GFX13-NEXT:    global_store_b8 v[0:1], v6, off offset:4
; GFX13-NEXT:    global_store_b32 v[0:1], v2, off
; GFX13-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = trunc <5 x i32> %val to <5 x i8>
  store <5 x i8> %trunc, ptr addrspace(1) %out
  ret void
}

define void @truncstore_v6i32_to_v6i1(ptr addrspace(1) %out, <6 x i32> %val) {
; SI-LABEL: truncstore_v6i32_to_v6i1:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_and_b32_e32 v3, 1, v3
; SI-NEXT:    v_and_b32_e32 v2, 1, v2
; SI-NEXT:    v_lshlrev_b32_e32 v3, 1, v3
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    v_and_b32_e32 v3, 1, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 2, v3
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    v_and_b32_e32 v3, 1, v5
; SI-NEXT:    v_lshlrev_b32_e32 v3, 3, v3
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    v_and_b32_e32 v3, 1, v6
; SI-NEXT:    v_lshlrev_b32_e32 v3, 4, v3
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    v_lshlrev_b32_e32 v3, 5, v7
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_and_b32_e32 v2, 63, v2
; SI-NEXT:    buffer_store_byte v2, v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: truncstore_v6i32_to_v6i1:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    v_and_b32_e32 v3, 1, v3
; VI-NEXT:    v_and_b32_e32 v2, 1, v2
; VI-NEXT:    v_lshlrev_b16_e32 v3, 1, v3
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_and_b32_e32 v3, 1, v4
; VI-NEXT:    v_lshlrev_b16_e32 v3, 2, v3
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_and_b32_e32 v3, 1, v5
; VI-NEXT:    v_lshlrev_b16_e32 v3, 3, v3
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_and_b32_e32 v3, 1, v6
; VI-NEXT:    v_lshlrev_b16_e32 v3, 4, v3
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_lshlrev_b16_e32 v3, 5, v7
; VI-NEXT:    v_or_b32_e32 v2, v2, v3
; VI-NEXT:    v_and_b32_e32 v2, 63, v2
; VI-NEXT:    flat_store_byte v[0:1], v2
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-TRUE16-LABEL: truncstore_v6i32_to_v6i1:
; GFX13-TRUE16:       ; %bb.0:
; GFX13-TRUE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_expcnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-TRUE16-NEXT:    v_and_b16 v2.h, v3.l, 1
; GFX13-TRUE16-NEXT:    v_and_b16 v3.l, v4.l, 1
; GFX13-TRUE16-NEXT:    v_and_b16 v2.l, v2.l, 1
; GFX13-TRUE16-NEXT:    v_and_b16 v3.h, v5.l, 1
; GFX13-TRUE16-NEXT:    v_and_b16 v4.l, v6.l, 1
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v2.h, 1, v2.h
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v3.l, 2, v3.l
; GFX13-TRUE16-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v3.h, 3, v3.h
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v4.l, 4, v4.l
; GFX13-TRUE16-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_2)
; GFX13-TRUE16-NEXT:    v_bitop3_b16 v2.l, v2.l, v3.l, v2.h bitop3:0xfe
; GFX13-TRUE16-NEXT:    v_lshlrev_b16 v2.h, 5, v7.l
; GFX13-TRUE16-NEXT:    v_bitop3_b16 v2.l, v2.l, v4.l, v3.h bitop3:0xfe
; GFX13-TRUE16-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX13-TRUE16-NEXT:    v_bitop3_b16 v2.l, v2.l, 63, v2.h bitop3:0xc8
; GFX13-TRUE16-NEXT:    global_store_b8 v[0:1], v2, off
; GFX13-TRUE16-NEXT:    s_set_pc_i64 s[30:31]
;
; GFX13-FAKE16-LABEL: truncstore_v6i32_to_v6i1:
; GFX13-FAKE16:       ; %bb.0:
; GFX13-FAKE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_expcnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v3, 1, v3
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v4, 1, v4
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v2, 1, v2
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v5, 1, v5
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v6, 1, v6
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v3, 1, v3
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v4, 2, v4
; GFX13-FAKE16-NEXT:    s_delay_alu instid0(VALU_DEP_4) | instskip(NEXT) | instid1(VALU_DEP_4)
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v5, 3, v5
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v6, 4, v6
; GFX13-FAKE16-NEXT:    s_delay_alu instid0(VALU_DEP_3) | instskip(SKIP_1) | instid1(VALU_DEP_2)
; GFX13-FAKE16-NEXT:    v_bitop3_b16 v2, v2, v4, v3 bitop3:0xfe
; GFX13-FAKE16-NEXT:    v_lshlrev_b16 v3, 5, v7
; GFX13-FAKE16-NEXT:    v_bitop3_b16 v2, v2, v6, v5 bitop3:0xfe
; GFX13-FAKE16-NEXT:    s_delay_alu instid0(VALU_DEP_1)
; GFX13-FAKE16-NEXT:    v_bitop3_b16 v2, v2, 63, v3 bitop3:0xc8
; GFX13-FAKE16-NEXT:    global_store_b8 v[0:1], v2, off
; GFX13-FAKE16-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = trunc <6 x i32> %val to <6 x i1>
  store <6 x i1> %trunc, ptr addrspace(1) %out
  ret void
}

define void @truncstore_v6i32_to_v6i8(ptr addrspace(1) %out, <6 x i32> %val) {
; SI-LABEL: truncstore_v6i32_to_v6i8:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_and_b32_e32 v4, 0xff, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 8, v3
; SI-NEXT:    v_and_b32_e32 v2, 0xff, v2
; SI-NEXT:    v_lshlrev_b32_e32 v5, 24, v5
; SI-NEXT:    v_lshlrev_b32_e32 v4, 16, v4
; SI-NEXT:    v_or_b32_e32 v2, v2, v3
; SI-NEXT:    v_or_b32_e32 v4, v5, v4
; SI-NEXT:    v_and_b32_e32 v2, 0xffff, v2
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_or_b32_e32 v2, v2, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 8, v7
; SI-NEXT:    v_and_b32_e32 v4, 0xff, v6
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_or_b32_e32 v3, v4, v3
; SI-NEXT:    buffer_store_short v3, v[0:1], s[4:7], 0 addr64 offset:4
; SI-NEXT:    buffer_store_dword v2, v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: truncstore_v6i32_to_v6i8:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    s_mov_b32 s4, 0xc0c0004
; VI-NEXT:    v_perm_b32 v2, v2, v3, s4
; VI-NEXT:    v_perm_b32 v3, v4, v5, s4
; VI-NEXT:    v_lshlrev_b32_e32 v3, 16, v3
; VI-NEXT:    v_or_b32_e32 v4, v2, v3
; VI-NEXT:    v_add_u32_e32 v2, vcc, 4, v0
; VI-NEXT:    v_perm_b32 v5, v6, v7, s4
; VI-NEXT:    v_addc_u32_e32 v3, vcc, 0, v1, vcc
; VI-NEXT:    flat_store_short v[2:3], v5
; VI-NEXT:    flat_store_dword v[0:1], v4
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-LABEL: truncstore_v6i32_to_v6i8:
; GFX13:       ; %bb.0:
; GFX13-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-NEXT:    s_wait_expcnt 0x0
; GFX13-NEXT:    s_wait_samplecnt 0x0
; GFX13-NEXT:    s_wait_rtscnt 0x0
; GFX13-NEXT:    s_wait_kmcnt 0x0
; GFX13-NEXT:    v_perm_b32 v2, v2, v3, 0xc0c0004
; GFX13-NEXT:    v_perm_b32 v3, v4, v5, 0xc0c0004
; GFX13-NEXT:    v_perm_b32 v4, v6, v7, 0xc0c0004
; GFX13-NEXT:    s_delay_alu instid0(VALU_DEP_2)
; GFX13-NEXT:    v_lshl_or_b32 v2, v3, 16, v2
; GFX13-NEXT:    s_clause 0x1
; GFX13-NEXT:    global_store_b16 v[0:1], v4, off offset:4
; GFX13-NEXT:    global_store_b32 v[0:1], v2, off
; GFX13-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = trunc <6 x i32> %val to <6 x i8>
  store <6 x i8> %trunc, ptr addrspace(1) %out
  ret void
}

define void @truncstore_v6i32_to_v6i16(ptr addrspace(1) %out, <6 x i32> %val) {
; SI-LABEL: truncstore_v6i32_to_v6i16:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_lshlrev_b32_e32 v5, 16, v5
; SI-NEXT:    v_and_b32_e32 v4, 0xffff, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 16, v3
; SI-NEXT:    v_and_b32_e32 v2, 0xffff, v2
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_or_b32_e32 v4, v4, v5
; SI-NEXT:    v_or_b32_e32 v3, v2, v3
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v7
; SI-NEXT:    v_and_b32_e32 v5, 0xffff, v6
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_or_b32_e32 v2, v5, v2
; SI-NEXT:    buffer_store_dword v2, v[0:1], s[4:7], 0 addr64 offset:8
; SI-NEXT:    buffer_store_dwordx2 v[3:4], v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: truncstore_v6i32_to_v6i16:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    s_mov_b32 s4, 0x1000504
; VI-NEXT:    v_perm_b32 v6, v6, v7, s4
; VI-NEXT:    v_perm_b32 v5, v4, v5, s4
; VI-NEXT:    v_perm_b32 v4, v2, v3, s4
; VI-NEXT:    flat_store_dwordx3 v[0:1], v[4:6]
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-TRUE16-LABEL: truncstore_v6i32_to_v6i16:
; GFX13-TRUE16:       ; %bb.0:
; GFX13-TRUE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_expcnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v10, v6 :: v_dual_mov_b32 v9, v4
; GFX13-TRUE16-NEXT:    v_mov_b32_e32 v8, v2
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v10.h, v7.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v9.h, v5.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v8.h, v3.l
; GFX13-TRUE16-NEXT:    global_store_b96 v[0:1], v[8:10], off
; GFX13-TRUE16-NEXT:    s_set_pc_i64 s[30:31]
;
; GFX13-FAKE16-LABEL: truncstore_v6i32_to_v6i16:
; GFX13-FAKE16:       ; %bb.0:
; GFX13-FAKE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_expcnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-FAKE16-NEXT:    v_perm_b32 v6, v7, v6, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v5, v5, v4, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v4, v3, v2, 0x5040100
; GFX13-FAKE16-NEXT:    global_store_b96 v[0:1], v[4:6], off
; GFX13-FAKE16-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = trunc <6 x i32> %val to <6 x i16>
  store <6 x i16> %trunc, ptr addrspace(1) %out
  ret void
}

define void @truncstore_v10i32_to_v10i16(ptr addrspace(1) %out, <10 x i32> %val) {
; SI-LABEL: truncstore_v10i32_to_v10i16:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_lshlrev_b32_e32 v7, 16, v7
; SI-NEXT:    v_and_b32_e32 v6, 0xffff, v6
; SI-NEXT:    v_lshlrev_b32_e32 v5, 16, v5
; SI-NEXT:    v_and_b32_e32 v4, 0xffff, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 16, v3
; SI-NEXT:    v_and_b32_e32 v2, 0xffff, v2
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_or_b32_e32 v7, v6, v7
; SI-NEXT:    v_or_b32_e32 v6, v4, v5
; SI-NEXT:    v_or_b32_e32 v5, v2, v3
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v11
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v10
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_lshlrev_b32_e32 v9, 16, v9
; SI-NEXT:    v_and_b32_e32 v8, 0xffff, v8
; SI-NEXT:    v_or_b32_e32 v2, v3, v2
; SI-NEXT:    v_or_b32_e32 v8, v8, v9
; SI-NEXT:    buffer_store_dword v2, v[0:1], s[4:7], 0 addr64 offset:16
; SI-NEXT:    buffer_store_dwordx4 v[5:8], v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: truncstore_v10i32_to_v10i16:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    s_mov_b32 s4, 0x1000504
; VI-NEXT:    v_perm_b32 v7, v6, v7, s4
; VI-NEXT:    v_perm_b32 v6, v4, v5, s4
; VI-NEXT:    v_perm_b32 v5, v2, v3, s4
; VI-NEXT:    v_add_u32_e32 v2, vcc, 16, v0
; VI-NEXT:    v_perm_b32 v4, v10, v11, s4
; VI-NEXT:    v_addc_u32_e32 v3, vcc, 0, v1, vcc
; VI-NEXT:    v_perm_b32 v8, v8, v9, s4
; VI-NEXT:    flat_store_dword v[2:3], v4
; VI-NEXT:    flat_store_dwordx4 v[0:1], v[5:8]
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-TRUE16-LABEL: truncstore_v10i32_to_v10i16:
; GFX13-TRUE16:       ; %bb.0:
; GFX13-TRUE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_expcnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v15, v8 :: v_dual_mov_b32 v14, v6
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v13, v4 :: v_dual_mov_b32 v12, v2
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v10.h, v11.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v15.h, v9.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v14.h, v7.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v13.h, v5.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v12.h, v3.l
; GFX13-TRUE16-NEXT:    s_clause 0x1
; GFX13-TRUE16-NEXT:    global_store_b32 v[0:1], v10, off offset:16
; GFX13-TRUE16-NEXT:    global_store_b128 v[0:1], v[12:15], off
; GFX13-TRUE16-NEXT:    s_set_pc_i64 s[30:31]
;
; GFX13-FAKE16-LABEL: truncstore_v10i32_to_v10i16:
; GFX13-FAKE16:       ; %bb.0:
; GFX13-FAKE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_expcnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-FAKE16-NEXT:    v_perm_b32 v7, v7, v6, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v6, v5, v4, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v4, v11, v10, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v8, v9, v8, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v5, v3, v2, 0x5040100
; GFX13-FAKE16-NEXT:    s_clause 0x1
; GFX13-FAKE16-NEXT:    global_store_b32 v[0:1], v4, off offset:16
; GFX13-FAKE16-NEXT:    global_store_b128 v[0:1], v[5:8], off
; GFX13-FAKE16-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = trunc <10 x i32> %val to <10 x i16>
  store <10 x i16> %trunc, ptr addrspace(1) %out
  ret void
}

define void @truncstore_v18i32_to_v18i16(ptr addrspace(1) %out, <18 x i32> %val) {
; SI-LABEL: truncstore_v18i32_to_v18i16:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_lshlrev_b32_e32 v5, 16, v5
; SI-NEXT:    v_and_b32_e32 v4, 0xffff, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 16, v3
; SI-NEXT:    v_and_b32_e32 v2, 0xffff, v2
; SI-NEXT:    v_or_b32_e32 v5, v4, v5
; SI-NEXT:    v_or_b32_e32 v4, v2, v3
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v17
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v16
; SI-NEXT:    v_or_b32_e32 v16, v3, v2
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v15
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v14
; SI-NEXT:    v_or_b32_e32 v15, v3, v2
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v13
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v12
; SI-NEXT:    v_or_b32_e32 v14, v3, v2
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v11
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v10
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_or_b32_e32 v13, v3, v2
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v19
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v18
; SI-NEXT:    v_lshlrev_b32_e32 v9, 16, v9
; SI-NEXT:    v_and_b32_e32 v8, 0xffff, v8
; SI-NEXT:    v_lshlrev_b32_e32 v20, 16, v7
; SI-NEXT:    v_and_b32_e32 v6, 0xffff, v6
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_or_b32_e32 v2, v3, v2
; SI-NEXT:    v_or_b32_e32 v7, v8, v9
; SI-NEXT:    v_or_b32_e32 v6, v6, v20
; SI-NEXT:    buffer_store_dword v2, v[0:1], s[4:7], 0 addr64 offset:32
; SI-NEXT:    buffer_store_dwordx4 v[13:16], v[0:1], s[4:7], 0 addr64 offset:16
; SI-NEXT:    buffer_store_dwordx4 v[4:7], v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: truncstore_v18i32_to_v18i16:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    s_mov_b32 s4, 0x1000504
; VI-NEXT:    v_perm_b32 v7, v6, v7, s4
; VI-NEXT:    v_perm_b32 v6, v4, v5, s4
; VI-NEXT:    v_perm_b32 v5, v2, v3, s4
; VI-NEXT:    v_add_u32_e32 v2, vcc, 16, v0
; VI-NEXT:    v_perm_b32 v16, v16, v17, s4
; VI-NEXT:    v_perm_b32 v15, v14, v15, s4
; VI-NEXT:    v_perm_b32 v14, v12, v13, s4
; VI-NEXT:    v_perm_b32 v13, v10, v11, s4
; VI-NEXT:    v_addc_u32_e32 v3, vcc, 0, v1, vcc
; VI-NEXT:    v_perm_b32 v8, v8, v9, s4
; VI-NEXT:    flat_store_dwordx4 v[2:3], v[13:16]
; VI-NEXT:    flat_store_dwordx4 v[0:1], v[5:8]
; VI-NEXT:    v_add_u32_e32 v0, vcc, 32, v0
; VI-NEXT:    v_perm_b32 v9, v18, v19, s4
; VI-NEXT:    v_addc_u32_e32 v1, vcc, 0, v1, vcc
; VI-NEXT:    flat_store_dword v[0:1], v9
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-TRUE16-LABEL: truncstore_v18i32_to_v18i16:
; GFX13-TRUE16:       ; %bb.0:
; GFX13-TRUE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_expcnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v23, v16 :: v_dual_mov_b32 v22, v14
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v21, v12 :: v_dual_mov_b32 v20, v10
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v18.h, v19.l
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v27, v8 :: v_dual_mov_b32 v26, v6
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v25, v4 :: v_dual_mov_b32 v24, v2
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v23.h, v17.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v22.h, v15.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v21.h, v13.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v20.h, v11.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v27.h, v9.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v26.h, v7.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v25.h, v5.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v24.h, v3.l
; GFX13-TRUE16-NEXT:    s_clause 0x2
; GFX13-TRUE16-NEXT:    global_store_b32 v[0:1], v18, off offset:32
; GFX13-TRUE16-NEXT:    global_store_b128 v[0:1], v[20:23], off offset:16
; GFX13-TRUE16-NEXT:    global_store_b128 v[0:1], v[24:27], off
; GFX13-TRUE16-NEXT:    s_set_pc_i64 s[30:31]
;
; GFX13-FAKE16-LABEL: truncstore_v18i32_to_v18i16:
; GFX13-FAKE16:       ; %bb.0:
; GFX13-FAKE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_expcnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-FAKE16-NEXT:    v_perm_b32 v7, v7, v6, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v6, v5, v4, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v4, v19, v18, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v16, v17, v16, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v15, v15, v14, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v14, v13, v12, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v13, v11, v10, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v8, v9, v8, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v5, v3, v2, 0x5040100
; GFX13-FAKE16-NEXT:    s_clause 0x2
; GFX13-FAKE16-NEXT:    global_store_b32 v[0:1], v4, off offset:32
; GFX13-FAKE16-NEXT:    global_store_b128 v[0:1], v[13:16], off offset:16
; GFX13-FAKE16-NEXT:    global_store_b128 v[0:1], v[5:8], off
; GFX13-FAKE16-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = trunc <18 x i32> %val to <18 x i16>
  store <18 x i16> %trunc, ptr addrspace(1) %out
  ret void
}

define void @truncstore_v36i32_to_v36i16(ptr addrspace(1) %out, <36 x i32> %val) {
; SI-LABEL: truncstore_v36i32_to_v36i16:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_lshlrev_b32_e32 v5, 16, v5
; SI-NEXT:    v_and_b32_e32 v4, 0xffff, v4
; SI-NEXT:    v_lshlrev_b32_e32 v3, 16, v3
; SI-NEXT:    v_and_b32_e32 v2, 0xffff, v2
; SI-NEXT:    v_lshlrev_b32_e32 v29, 16, v29
; SI-NEXT:    v_and_b32_e32 v28, 0xffff, v28
; SI-NEXT:    v_lshlrev_b32_e32 v9, 16, v9
; SI-NEXT:    v_and_b32_e32 v8, 0xffff, v8
; SI-NEXT:    v_or_b32_e32 v5, v4, v5
; SI-NEXT:    v_or_b32_e32 v4, v2, v3
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v17
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v16
; SI-NEXT:    v_lshlrev_b32_e32 v31, 16, v27
; SI-NEXT:    v_or_b32_e32 v27, v28, v29
; SI-NEXT:    v_lshlrev_b32_e32 v28, 16, v7
; SI-NEXT:    v_or_b32_e32 v7, v8, v9
; SI-NEXT:    v_lshlrev_b32_e32 v8, 16, v15
; SI-NEXT:    v_and_b32_e32 v9, 0xffff, v14
; SI-NEXT:    v_or_b32_e32 v15, v3, v2
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v13
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v12
; SI-NEXT:    v_or_b32_e32 v14, v9, v8
; SI-NEXT:    v_or_b32_e32 v13, v3, v2
; SI-NEXT:    buffer_load_dword v2, off, s[0:3], s32 offset:8
; SI-NEXT:    buffer_load_dword v3, off, s[0:3], s32 offset:4
; SI-NEXT:    buffer_load_dword v8, off, s[0:3], s32
; SI-NEXT:    v_and_b32_e32 v26, 0xffff, v26
; SI-NEXT:    v_or_b32_e32 v26, v26, v31
; SI-NEXT:    buffer_load_dword v16, off, s[0:3], s32 offset:24
; SI-NEXT:    buffer_load_dword v17, off, s[0:3], s32 offset:20
; SI-NEXT:    buffer_load_dword v31, off, s[0:3], s32 offset:16
; SI-NEXT:    buffer_load_dword v32, off, s[0:3], s32 offset:12
; SI-NEXT:    v_lshlrev_b32_e32 v9, 16, v11
; SI-NEXT:    v_and_b32_e32 v10, 0xffff, v10
; SI-NEXT:    v_and_b32_e32 v6, 0xffff, v6
; SI-NEXT:    v_or_b32_e32 v12, v10, v9
; SI-NEXT:    v_and_b32_e32 v9, 0xffff, v24
; SI-NEXT:    v_or_b32_e32 v6, v6, v28
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_lshlrev_b32_e32 v10, 16, v23
; SI-NEXT:    v_and_b32_e32 v22, 0xffff, v22
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_or_b32_e32 v10, v22, v10
; SI-NEXT:    s_waitcnt vmcnt(6)
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v2
; SI-NEXT:    s_waitcnt vmcnt(5)
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v3
; SI-NEXT:    v_or_b32_e32 v29, v3, v2
; SI-NEXT:    v_and_b32_e32 v2, 0xffff, v30
; SI-NEXT:    v_lshlrev_b32_e32 v3, 16, v25
; SI-NEXT:    s_waitcnt vmcnt(4)
; SI-NEXT:    v_lshlrev_b32_e32 v8, 16, v8
; SI-NEXT:    v_or_b32_e32 v28, v2, v8
; SI-NEXT:    v_or_b32_e32 v11, v9, v3
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v21
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v20
; SI-NEXT:    v_or_b32_e32 v9, v3, v2
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v19
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v18
; SI-NEXT:    v_or_b32_e32 v8, v3, v2
; SI-NEXT:    s_waitcnt vmcnt(3)
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v16
; SI-NEXT:    s_waitcnt vmcnt(2)
; SI-NEXT:    v_and_b32_e32 v3, 0xffff, v17
; SI-NEXT:    s_waitcnt vmcnt(1)
; SI-NEXT:    v_lshlrev_b32_e32 v16, 16, v31
; SI-NEXT:    s_waitcnt vmcnt(0)
; SI-NEXT:    v_and_b32_e32 v17, 0xffff, v32
; SI-NEXT:    buffer_store_dwordx4 v[26:29], v[0:1], s[4:7], 0 addr64 offset:48
; SI-NEXT:    v_or_b32_e32 v3, v3, v2
; SI-NEXT:    v_or_b32_e32 v2, v17, v16
; SI-NEXT:    buffer_store_dwordx4 v[8:11], v[0:1], s[4:7], 0 addr64 offset:32
; SI-NEXT:    buffer_store_dwordx4 v[12:15], v[0:1], s[4:7], 0 addr64 offset:16
; SI-NEXT:    buffer_store_dwordx4 v[4:7], v[0:1], s[4:7], 0 addr64
; SI-NEXT:    buffer_store_dwordx2 v[2:3], v[0:1], s[4:7], 0 addr64 offset:64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: truncstore_v36i32_to_v36i16:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    s_mov_b32 s4, 0x1000504
; VI-NEXT:    v_perm_b32 v23, v22, v23, s4
; VI-NEXT:    v_perm_b32 v22, v20, v21, s4
; VI-NEXT:    v_perm_b32 v21, v18, v19, s4
; VI-NEXT:    v_add_u32_e32 v18, vcc, 32, v0
; VI-NEXT:    v_perm_b32 v24, v24, v25, s4
; VI-NEXT:    v_addc_u32_e32 v19, vcc, 0, v1, vcc
; VI-NEXT:    flat_store_dwordx4 v[18:19], v[21:24]
; VI-NEXT:    buffer_load_dword v19, off, s[0:3], s32 offset:16
; VI-NEXT:    buffer_load_dword v20, off, s[0:3], s32 offset:12
; VI-NEXT:    buffer_load_dword v21, off, s[0:3], s32 offset:24
; VI-NEXT:    buffer_load_dword v22, off, s[0:3], s32 offset:20
; VI-NEXT:    buffer_load_dword v23, off, s[0:3], s32 offset:8
; VI-NEXT:    buffer_load_dword v24, off, s[0:3], s32 offset:4
; VI-NEXT:    buffer_load_dword v25, off, s[0:3], s32
; VI-NEXT:    v_perm_b32 v8, v8, v9, s4
; VI-NEXT:    v_add_u32_e32 v9, vcc, 16, v0
; VI-NEXT:    v_perm_b32 v15, v14, v15, s4
; VI-NEXT:    v_perm_b32 v14, v12, v13, s4
; VI-NEXT:    v_perm_b32 v13, v10, v11, s4
; VI-NEXT:    v_addc_u32_e32 v10, vcc, 0, v1, vcc
; VI-NEXT:    v_add_u32_e32 v11, vcc, 48, v0
; VI-NEXT:    v_perm_b32 v16, v16, v17, s4
; VI-NEXT:    v_perm_b32 v7, v6, v7, s4
; VI-NEXT:    v_perm_b32 v6, v4, v5, s4
; VI-NEXT:    v_perm_b32 v5, v2, v3, s4
; VI-NEXT:    v_addc_u32_e32 v12, vcc, 0, v1, vcc
; VI-NEXT:    v_perm_b32 v3, v28, v29, s4
; VI-NEXT:    v_perm_b32 v2, v26, v27, s4
; VI-NEXT:    v_add_u32_e32 v17, vcc, 64, v0
; VI-NEXT:    flat_store_dwordx4 v[9:10], v[13:16]
; VI-NEXT:    flat_store_dwordx4 v[0:1], v[5:8]
; VI-NEXT:    v_addc_u32_e32 v18, vcc, 0, v1, vcc
; VI-NEXT:    s_waitcnt vmcnt(7)
; VI-NEXT:    v_perm_b32 v0, v20, v19, s4
; VI-NEXT:    s_waitcnt vmcnt(5)
; VI-NEXT:    v_perm_b32 v1, v22, v21, s4
; VI-NEXT:    s_waitcnt vmcnt(3)
; VI-NEXT:    v_perm_b32 v5, v24, v23, s4
; VI-NEXT:    s_waitcnt vmcnt(2)
; VI-NEXT:    v_perm_b32 v4, v30, v25, s4
; VI-NEXT:    flat_store_dwordx4 v[11:12], v[2:5]
; VI-NEXT:    flat_store_dwordx2 v[17:18], v[0:1]
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-TRUE16-LABEL: truncstore_v36i32_to_v36i16:
; GFX13-TRUE16:       ; %bb.0:
; GFX13-TRUE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_expcnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-TRUE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v48, v10 :: v_dual_mov_b32 v55, v8
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v54, v6 :: v_dual_mov_b32 v53, v4
; GFX13-TRUE16-NEXT:    v_mov_b32_e32 v52, v2
; GFX13-TRUE16-NEXT:    s_clause 0x6
; GFX13-TRUE16-NEXT:    scratch_load_b32 v2, off, s32 offset:8
; GFX13-TRUE16-NEXT:    scratch_load_b32 v4, off, s32
; GFX13-TRUE16-NEXT:    scratch_load_b32 v6, off, s32 offset:24
; GFX13-TRUE16-NEXT:    scratch_load_b32 v8, off, s32 offset:16
; GFX13-TRUE16-NEXT:    scratch_load_b32 v33, off, s32 offset:4
; GFX13-TRUE16-NEXT:    scratch_load_b32 v38, off, s32 offset:12
; GFX13-TRUE16-NEXT:    scratch_load_b32 v39, off, s32 offset:20
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v34, v18 :: v_dual_mov_b32 v51, v16
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v50, v14 :: v_dual_mov_b32 v49, v12
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v51.h, v17.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v50.h, v15.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v49.h, v13.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v48.h, v11.l
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v32, v30 :: v_dual_mov_b32 v31, v28
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v30, v26 :: v_dual_mov_b32 v37, v24
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v55.h, v9.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v54.h, v7.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v53.h, v5.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v52.h, v3.l
; GFX13-TRUE16-NEXT:    v_dual_mov_b32 v36, v22 :: v_dual_mov_b32 v35, v20
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v31.h, v29.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v30.h, v27.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v37.h, v25.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v36.h, v23.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v35.h, v21.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v34.h, v19.l
; GFX13-TRUE16-NEXT:    s_wait_loadcnt 0x2
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v33.h, v2.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v32.h, v4.l
; GFX13-TRUE16-NEXT:    s_wait_loadcnt 0x0
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v39.h, v6.l
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v38.h, v8.l
; GFX13-TRUE16-NEXT:    s_clause 0x4
; GFX13-TRUE16-NEXT:    global_store_b128 v[0:1], v[48:51], off offset:16
; GFX13-TRUE16-NEXT:    global_store_b128 v[0:1], v[52:55], off
; GFX13-TRUE16-NEXT:    global_store_b128 v[0:1], v[30:33], off offset:48
; GFX13-TRUE16-NEXT:    global_store_b128 v[0:1], v[34:37], off offset:32
; GFX13-TRUE16-NEXT:    global_store_b64 v[0:1], v[38:39], off offset:64
; GFX13-TRUE16-NEXT:    s_set_pc_i64 s[30:31]
;
; GFX13-FAKE16-LABEL: truncstore_v36i32_to_v36i16:
; GFX13-FAKE16:       ; %bb.0:
; GFX13-FAKE16-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_expcnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_samplecnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_rtscnt 0x0
; GFX13-FAKE16-NEXT:    s_wait_kmcnt 0x0
; GFX13-FAKE16-NEXT:    s_clause 0x6
; GFX13-FAKE16-NEXT:    scratch_load_b32 v31, off, s32
; GFX13-FAKE16-NEXT:    scratch_load_b32 v32, off, s32 offset:8
; GFX13-FAKE16-NEXT:    scratch_load_b32 v33, off, s32 offset:4
; GFX13-FAKE16-NEXT:    scratch_load_b32 v34, off, s32 offset:16
; GFX13-FAKE16-NEXT:    scratch_load_b32 v35, off, s32 offset:24
; GFX13-FAKE16-NEXT:    scratch_load_b32 v36, off, s32 offset:20
; GFX13-FAKE16-NEXT:    scratch_load_b32 v37, off, s32 offset:12
; GFX13-FAKE16-NEXT:    v_perm_b32 v16, v17, v16, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v15, v15, v14, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v14, v13, v12, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v13, v11, v10, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v8, v9, v8, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v7, v7, v6, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v6, v5, v4, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v5, v3, v2, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v10, v21, v20, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v9, v19, v18, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v18, v29, v28, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v17, v27, v26, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v12, v25, v24, 0x5040100
; GFX13-FAKE16-NEXT:    v_perm_b32 v11, v23, v22, 0x5040100
; GFX13-FAKE16-NEXT:    s_wait_loadcnt 0x6
; GFX13-FAKE16-NEXT:    v_perm_b32 v19, v31, v30, 0x5040100
; GFX13-FAKE16-NEXT:    s_wait_loadcnt 0x4
; GFX13-FAKE16-NEXT:    v_perm_b32 v20, v32, v33, 0x5040100
; GFX13-FAKE16-NEXT:    s_wait_loadcnt 0x1
; GFX13-FAKE16-NEXT:    v_perm_b32 v3, v35, v36, 0x5040100
; GFX13-FAKE16-NEXT:    s_wait_loadcnt 0x0
; GFX13-FAKE16-NEXT:    v_perm_b32 v2, v34, v37, 0x5040100
; GFX13-FAKE16-NEXT:    s_clause 0x4
; GFX13-FAKE16-NEXT:    global_store_b128 v[0:1], v[13:16], off offset:16
; GFX13-FAKE16-NEXT:    global_store_b128 v[0:1], v[5:8], off
; GFX13-FAKE16-NEXT:    global_store_b128 v[0:1], v[17:20], off offset:48
; GFX13-FAKE16-NEXT:    global_store_b128 v[0:1], v[9:12], off offset:32
; GFX13-FAKE16-NEXT:    global_store_b64 v[0:1], v[2:3], off offset:64
; GFX13-FAKE16-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = trunc <36 x i32> %val to <36 x i16>
  store <36 x i16> %trunc, ptr addrspace(1) %out
  ret void
}
define void @global_fp_truncstore_v6f32_to_v6bf16(ptr addrspace(1) %ptr, <6 x float> %src) {
; SI-LABEL: global_fp_truncstore_v6f32_to_v6bf16:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_mul_f32_e32 v3, 1.0, v3
; SI-NEXT:    v_mul_f32_e32 v5, 1.0, v5
; SI-NEXT:    v_lshrrev_b32_e32 v3, 16, v3
; SI-NEXT:    v_mul_f32_e32 v2, 1.0, v2
; SI-NEXT:    v_lshrrev_b32_e32 v5, 16, v5
; SI-NEXT:    v_mul_f32_e32 v4, 1.0, v4
; SI-NEXT:    v_alignbit_b32 v3, v3, v2, 16
; SI-NEXT:    v_mul_f32_e32 v2, 1.0, v7
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_alignbit_b32 v4, v5, v4, 16
; SI-NEXT:    v_lshrrev_b32_e32 v2, 16, v2
; SI-NEXT:    v_mul_f32_e32 v5, 1.0, v6
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_alignbit_b32 v2, v2, v5, 16
; SI-NEXT:    buffer_store_dword v2, v[0:1], s[4:7], 0 addr64 offset:8
; SI-NEXT:    buffer_store_dwordx2 v[3:4], v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: global_fp_truncstore_v6f32_to_v6bf16:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    v_bfe_u32 v9, v6, 16, 1
; VI-NEXT:    v_add_u32_e32 v9, vcc, v9, v6
; VI-NEXT:    v_add_u32_e32 v9, vcc, 0x7fff, v9
; VI-NEXT:    v_or_b32_e32 v8, 0x400000, v6
; VI-NEXT:    v_cmp_u_f32_e32 vcc, v6, v6
; VI-NEXT:    v_cndmask_b32_e32 v6, v9, v8, vcc
; VI-NEXT:    v_bfe_u32 v9, v7, 16, 1
; VI-NEXT:    s_movk_i32 s4, 0x7fff
; VI-NEXT:    v_add_u32_e32 v9, vcc, v9, v7
; VI-NEXT:    v_add_u32_e32 v9, vcc, s4, v9
; VI-NEXT:    v_or_b32_e32 v8, 0x400000, v7
; VI-NEXT:    v_cmp_u_f32_e32 vcc, v7, v7
; VI-NEXT:    v_cndmask_b32_e32 v7, v9, v8, vcc
; VI-NEXT:    v_bfe_u32 v8, v4, 16, 1
; VI-NEXT:    v_add_u32_e32 v8, vcc, v8, v4
; VI-NEXT:    v_lshrrev_b32_e32 v7, 16, v7
; VI-NEXT:    v_add_u32_e32 v8, vcc, s4, v8
; VI-NEXT:    v_alignbit_b32 v6, v7, v6, 16
; VI-NEXT:    v_or_b32_e32 v7, 0x400000, v4
; VI-NEXT:    v_cmp_u_f32_e32 vcc, v4, v4
; VI-NEXT:    v_cndmask_b32_e32 v4, v8, v7, vcc
; VI-NEXT:    v_bfe_u32 v8, v5, 16, 1
; VI-NEXT:    v_add_u32_e32 v8, vcc, v8, v5
; VI-NEXT:    v_add_u32_e32 v8, vcc, s4, v8
; VI-NEXT:    v_or_b32_e32 v7, 0x400000, v5
; VI-NEXT:    v_cmp_u_f32_e32 vcc, v5, v5
; VI-NEXT:    v_cndmask_b32_e32 v5, v8, v7, vcc
; VI-NEXT:    v_bfe_u32 v7, v2, 16, 1
; VI-NEXT:    v_add_u32_e32 v7, vcc, v7, v2
; VI-NEXT:    v_lshrrev_b32_e32 v5, 16, v5
; VI-NEXT:    v_add_u32_e32 v7, vcc, 0x7fff, v7
; VI-NEXT:    v_alignbit_b32 v5, v5, v4, 16
; VI-NEXT:    v_or_b32_e32 v4, 0x400000, v2
; VI-NEXT:    v_cmp_u_f32_e32 vcc, v2, v2
; VI-NEXT:    v_cndmask_b32_e32 v2, v7, v4, vcc
; VI-NEXT:    v_bfe_u32 v7, v3, 16, 1
; VI-NEXT:    v_add_u32_e32 v7, vcc, v7, v3
; VI-NEXT:    v_add_u32_e32 v7, vcc, 0x7fff, v7
; VI-NEXT:    v_or_b32_e32 v4, 0x400000, v3
; VI-NEXT:    v_cmp_u_f32_e32 vcc, v3, v3
; VI-NEXT:    v_cndmask_b32_e32 v3, v7, v4, vcc
; VI-NEXT:    v_lshrrev_b32_e32 v3, 16, v3
; VI-NEXT:    v_alignbit_b32 v4, v3, v2, 16
; VI-NEXT:    flat_store_dwordx3 v[0:1], v[4:6]
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-LABEL: global_fp_truncstore_v6f32_to_v6bf16:
; GFX13:       ; %bb.0:
; GFX13-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-NEXT:    s_wait_expcnt 0x0
; GFX13-NEXT:    s_wait_samplecnt 0x0
; GFX13-NEXT:    s_wait_rtscnt 0x0
; GFX13-NEXT:    s_wait_kmcnt 0x0
; GFX13-NEXT:    v_cvt_pk_bf16_f32 v6, v6, v7
; GFX13-NEXT:    v_cvt_pk_bf16_f32 v5, v4, v5
; GFX13-NEXT:    v_cvt_pk_bf16_f32 v4, v2, v3
; GFX13-NEXT:    global_store_b96 v[0:1], v[4:6], off
; GFX13-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = fptrunc <6 x float> %src to <6 x bfloat>
  store <6 x bfloat> %trunc, ptr addrspace(1) %ptr
  ret void
}


define void @global_fp_truncstore_v6f32_to_v6f16(ptr addrspace(1) %ptr, <6 x float> %src) {
; SI-LABEL: global_fp_truncstore_v6f32_to_v6f16:
; SI:       ; %bb.0:
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SI-NEXT:    v_cvt_f16_f32_e32 v5, v5
; SI-NEXT:    v_cvt_f16_f32_e32 v4, v4
; SI-NEXT:    v_cvt_f16_f32_e32 v3, v3
; SI-NEXT:    v_cvt_f16_f32_e32 v2, v2
; SI-NEXT:    v_lshlrev_b32_e32 v5, 16, v5
; SI-NEXT:    v_or_b32_e32 v4, v4, v5
; SI-NEXT:    v_cvt_f16_f32_e32 v5, v7
; SI-NEXT:    v_cvt_f16_f32_e32 v6, v6
; SI-NEXT:    v_lshlrev_b32_e32 v3, 16, v3
; SI-NEXT:    s_mov_b32 s6, 0
; SI-NEXT:    v_or_b32_e32 v3, v2, v3
; SI-NEXT:    v_lshlrev_b32_e32 v2, 16, v5
; SI-NEXT:    s_mov_b32 s7, 0xf000
; SI-NEXT:    s_mov_b32 s4, s6
; SI-NEXT:    s_mov_b32 s5, s6
; SI-NEXT:    v_or_b32_e32 v2, v6, v2
; SI-NEXT:    buffer_store_dword v2, v[0:1], s[4:7], 0 addr64 offset:8
; SI-NEXT:    buffer_store_dwordx2 v[3:4], v[0:1], s[4:7], 0 addr64
; SI-NEXT:    s_waitcnt vmcnt(0) expcnt(0)
; SI-NEXT:    s_setpc_b64 s[30:31]
;
; VI-LABEL: global_fp_truncstore_v6f32_to_v6f16:
; VI:       ; %bb.0:
; VI-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; VI-NEXT:    v_cvt_f16_f32_sdwa v7, v7 dst_sel:WORD_1 dst_unused:UNUSED_PAD src0_sel:DWORD
; VI-NEXT:    v_cvt_f16_f32_e32 v6, v6
; VI-NEXT:    v_cvt_f16_f32_sdwa v5, v5 dst_sel:WORD_1 dst_unused:UNUSED_PAD src0_sel:DWORD
; VI-NEXT:    v_cvt_f16_f32_e32 v8, v4
; VI-NEXT:    v_cvt_f16_f32_sdwa v9, v3 dst_sel:WORD_1 dst_unused:UNUSED_PAD src0_sel:DWORD
; VI-NEXT:    v_cvt_f16_f32_e32 v2, v2
; VI-NEXT:    v_or_b32_e32 v4, v6, v7
; VI-NEXT:    v_or_b32_e32 v3, v8, v5
; VI-NEXT:    v_or_b32_e32 v2, v2, v9
; VI-NEXT:    flat_store_dwordx3 v[0:1], v[2:4]
; VI-NEXT:    s_waitcnt vmcnt(0)
; VI-NEXT:    s_setpc_b64 s[30:31]
;
; GFX13-LABEL: global_fp_truncstore_v6f32_to_v6f16:
; GFX13:       ; %bb.0:
; GFX13-NEXT:    s_wait_loadcnt_dscnt 0x0
; GFX13-NEXT:    s_wait_expcnt 0x0
; GFX13-NEXT:    s_wait_samplecnt 0x0
; GFX13-NEXT:    s_wait_rtscnt 0x0
; GFX13-NEXT:    s_wait_kmcnt 0x0
; GFX13-NEXT:    v_cvt_pk_f16_f32 v6, v6, v7
; GFX13-NEXT:    v_cvt_pk_f16_f32 v5, v4, v5
; GFX13-NEXT:    v_cvt_pk_f16_f32 v4, v2, v3
; GFX13-NEXT:    global_store_b96 v[0:1], v[4:6], off
; GFX13-NEXT:    s_set_pc_i64 s[30:31]
  %trunc = fptrunc <6 x float> %src to <6 x half>
  store <6 x half> %trunc, ptr addrspace(1) %ptr
  ret void
}

;; NOTE: These prefixes are unused and the list is autogenerated. Do not add tests below this line:
; GCN: {{.*}}

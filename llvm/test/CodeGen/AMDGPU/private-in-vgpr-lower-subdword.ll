...
; RUN: llc -mtriple=amdgcn-- -mcpu=gfx1300 -amdgpu-promote-private=true -mattr=-real-true16 -amdgpu-promote-sub-dword=true -stop-after=finalize-isel -verify-machineinstrs -o - %s | FileCheck -check-prefix=VIDX %s
; RUN: llc -mtriple=amdgcn-- -mcpu=gfx1300 -amdgpu-promote-private=true -mattr=-real-true16 -amdgpu-promote-sub-dword=true -stop-after=bundle-indexed-load-store -verify-machineinstrs -o - %s | FileCheck -check-prefix=BUNDLE %s
; RUN: llc -mtriple=amdgcn-- -mcpu=gfx1300 -amdgpu-promote-private=true -mattr=-real-true16 -amdgpu-promote-sub-dword=true -stop-after=amdgpu-idx-reg-alloc -verify-machineinstrs -o - %s | FileCheck -check-prefix=SETIDX %s
...
define amdgpu_kernel void @subdword() #0 {
  ...
  ; BUNDLE-NEXT:     V_STORE_IDX_B32 internal $stg_dsta, [[S_MOV_B32_3]], 0, implicit $exec :: (store (s16) into %ir.q.2, align 4, addrspace 5)
  ; BUNDLE-NEXT:   }

...
; CHECK-NEXT: %dispatch.ptr1 = tail call ptr addrspace(4) @llvm.amdgcn.dispatch.ptr()
; CHECK-NEXT: %gep.group.size.y = getelementptr inbounds i8, ptr addrspace(4) %dispatch.ptr1, i64 6
; CHECK-NEXT: %group.size.y = load i16, ptr addrspace(4) %gep.group.size.y, align 4
; CHECK-NEXT: store volatile i16 %group.size.y, ptr addrspace(1) %out, align 2
...
; CHECK-NEXT: %gep.grid.size.x = getelementptr inbounds i8, ptr addrspace(4) %dispatch.ptr, i64 12
; CHECK-NEXT: %grid.size.x = load i32, ptr addrspace(4) %gep.grid.size.x, align 4
; CHECK-NEXT: %group.id = tail call i32 @llvm.amdgcn.workgroup.id.x()
; CHECK-NEXT: %group.size.x.zext = zext i16 %group.size.x to i32
; CHECK-NEXT: %group.id_x_group.size.x = mul i32 %group.id, %group.size.x.zext
; CHECK-NEXT: %sub = sub i32 %grid.size.x, %group.id_x_group.size.x
; CHECK-NEXT: %umin = call i32 @llvm.umin.i32(i32 %sub, i32 %group.size.x.zext)
; CHECK-NEXT: %zext = zext i32 %umin to i64

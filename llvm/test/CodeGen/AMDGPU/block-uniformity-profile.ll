...
; profile classification is unknown. In these examples, the branches depend on
; uniform kernel arguments, so static analysis can still classify their blocks
; as uniformly reached without inventing a PGO annotation.
;
...
define amdgpu_kernel void @uniform_blocks(ptr addrspace(1) %out) #0 !uniformity.profile !0 {
entry:
...
; CHECK-NEXT: HasProfile: true
; CHECK-DAG: %bb.{{[0-9]+}} (%if.then): no PGO annotation (statically uniformly reached)
...
; CHECK-NEXT: HasProfile: true
; CHECK-DAG: %bb.{{[0-9]+}} (%loop.header): no PGO annotation (statically uniformly reached)
; CHECK-DAG: %bb.{{[0-9]+}} (%loop.body): no PGO annotation (statically uniformly reached)

; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: llc -mcpu=gfx1300 -mtriple=amdgcn-- -verify-machineinstrs -mattr=+real-true16 < %s | FileCheck -check-prefixes=GFX13,GFX13-TRUE16 %s
; RUN: llc -mcpu=gfx1300 -mtriple=amdgcn-- -verify-machineinstrs -mattr=-real-true16 < %s | FileCheck -check-prefixes=GFX13,GFX13-FAKE16 %s

@esgs_ring = external addrspace(3) global [0 x i32], align 65536

define amdgpu_gs void @main(i32 %arg, i32 %arg1) {
; GFX13-TRUE16-LABEL: main:
; GFX13-TRUE16:       ; %bb.0: ; %bb
; GFX13-TRUE16-NEXT:    buffer_load_d16_format_xyz v[0:1], v1, v0, null idxen
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v2.h, 0
; GFX13-TRUE16-NEXT:    s_wait_loadcnt 0x0
; GFX13-TRUE16-NEXT:    v_mov_b16_e32 v2.l, v0.h
; GFX13-TRUE16-NEXT:    v_and_b32_e32 v3, 0xffff, v1
; GFX13-TRUE16-NEXT:    v_mov_b32_e32 v0, 0
; GFX13-TRUE16-NEXT:    ds_store_b64 v0, v[2:3] offset:28
;
; GFX13-FAKE16-LABEL: main:
; GFX13-FAKE16:       ; %bb.0: ; %bb
; GFX13-FAKE16-NEXT:    buffer_load_d16_format_xyz v[0:1], v1, v0, null idxen
; GFX13-FAKE16-NEXT:    s_wait_loadcnt 0x0
; GFX13-FAKE16-NEXT:    v_dual_mov_b32 v2, 0 :: v_dual_lshrrev_b32 v0, 16, v0
; GFX13-FAKE16-NEXT:    v_and_b32_e32 v1, 0xffff, v1
; GFX13-FAKE16-NEXT:    ds_store_b64 v2, v[0:1] offset:28
bb:
  %i = call i32 @llvm.amdgcn.mbcnt.hi(i32 -1, i32 poison)
  %i2 = call nsz arcp <3 x half> @llvm.amdgcn.struct.buffer.load.format.v3f16(i32 %arg, i32 %arg1, i32 0, i32 0, i32 0)
  %i3 = bitcast <3 x half> %i2 to <3 x i16>
  %i4 = extractelement <3 x i16> %i3, i32 1
  %i5 = bitcast <3 x half> %i2 to <3 x i16>
  %i6 = extractelement <3 x i16> %i5, i32 2
  %i7 = zext i16 %i4 to i32
  %i8 = zext i16 %i6 to i32
  %i9 = add nuw nsw i32 0, 7
  %i10 = getelementptr [0 x i32], ptr addrspace(3) @esgs_ring, i32 0, i32 %i9
  store i32 %i7, ptr addrspace(3) %i10, align 4
  %i11 = add nuw nsw i32 0, 8
  %i12 = getelementptr [0 x i32], ptr addrspace(3) @esgs_ring, i32 0, i32 %i11
  store i32 %i8, ptr addrspace(3) %i12, align 4
  unreachable
}
; Function Attrs: nounwind readnone willreturn
declare i32 @llvm.amdgcn.mbcnt.hi(i32, i32) #0
; Function Attrs: nounwind readonly willreturn
declare <3 x half> @llvm.amdgcn.struct.buffer.load.format.v3f16(i32, i32, i32, i32, i32 immarg) #1
attributes #0 = { nounwind readnone willreturn }
attributes #1 = { nounwind readonly willreturn }
;; NOTE: These prefixes are unused and the list is autogenerated. Do not add tests below this line:
; GFX13: {{.*}}

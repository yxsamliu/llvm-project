...
; RUN: not llc -mtriple=amdgcn -mcpu=gfx1300 -filetype=null %t/spatial-cluster-1d-err.ll 2>&1 | FileCheck -check-prefix=CHECK-ERROR1 %s
; RUN: not llc -mtriple=amdgcn -mcpu=gfx1300 -filetype=null %t/spatial-cluster-wg-err.ll 2>&1 | FileCheck -check-prefix=CHECK-ERROR2 %s
; RUN: not llc -mtriple=amdgcn -mcpu=gfx1300 -filetype=null %t/spatial-cluster-fixed-dims-err.ll 2>&1 | FileCheck -check-prefix=CHECK-ERROR3 %s
...
; CHECK-ERROR2: error: Spatial cluster kernel is not wavegroup kernel
...
; CHECK-ERROR3: error: Spatial cluster kernel has non fixed cluster dims

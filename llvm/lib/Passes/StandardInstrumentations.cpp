...
static bool lineHasPrintedDebugLoc(StringRef Line) {
  size_t Semi = Line.rfind(';');
  if (Semi == StringRef::npos)
    return false;
  StringRef Suffix = Line.substr(Semi + 1).trim();
  size_t Colon3 = Suffix.rfind(':');
  if (Colon3 == StringRef::npos)
    return false;
  size_t Colon2 = Suffix.rfind(':', Colon3 - 1);
  if (Colon2 == StringRef::npos)
    return false;
  StringRef Middle = Suffix.substr(Colon2 + 1, Colon3 - Colon2 - 1);
  StringRef End = Suffix.substr(Colon3 + 1);
  return !Middle.empty() && !End.empty() &&
         std::all_of(Middle.begin(), Middle.end(),
                     [](unsigned char C) { return std::isdigit(C); }) &&
         std::all_of(End.begin(), End.end(),
                     [](unsigned char C) { return std::isdigit(C); });
}

static std::string filterConciseIRDump(StringRef Dump) {
  std::string Out;
  ...
  for (StringRef Line : Lines) {
    if (lineHasPrintedDebugLoc(Line)) {
      OS << Line << '\n';
      WroteAny = true;
    }
  }
  if (!WroteAny)
    OS << "; <no instructions with printed debug locations>\n";
  OS.flush();
  return Out;
}

static raw_ostream &getConciseIRPrintStream() {
  static std::optional<raw_fd_ostream> Stream;
  if (!Stream) {
    std::error_code EC;
    Stream.emplace(getConciseIRPrintOutputPath(), EC, sys::fs::OF_Text);
    if (EC)
      report_fatal_error(Twine("Failed to open concise IR print output: ") +
                         EC.message());
  }
  return *Stream;
}
...
public:
  explicit IRTrackerTextState(StringRef Path) {
    ...
    if (EC)
      report_fatal_error(Twine("ir-tracker text output open: ") + EC.message());
  }
...
void PrintIRInstrumentation::printAfterPass(StringRef PassID, Any IR) {
  ...
  auto WriteConciseIR = [&](raw_ostream &Stream, const StringRef IRName) {
    Stream << "; *** IR Dump After ";
    if (shouldPrintAfterSomePassNumber())
      Stream << CurrentPassNumber << "-";
    Stream << StringRef(formatv("{0}", PassID)) << " on " << IRName
           << " (concise) ***\n";
    std::string Dump;
    raw_string_ostream DumpOS(Dump);
    unwrapAndPrint(DumpOS, IR);
    DumpOS.flush();
    Stream << filterConciseIRDump(Dump);
  };

  if (!getConciseIRPrintOutputPath().empty()) {
    WriteConciseIR(getConciseIRPrintStream(), IRName);
    return;
  }
...
void IRTrackerInstrumentation::registerCallbacks(
    PassInstrumentationCallbacks &PIC) {
  StringRef TextPath = getIRTrackerTextOutputPath();
  if (!TextPath.empty()) {
    auto TextState = std::make_shared<IRTrackerTextState>(TextPath);
    PIC.registerBeforeNonSkippedPassCallback([TextState](StringRef PassID,
                                                         Any IR) {
      TextState->beforePass(PassID, IR);
    });
    PIC.registerAfterPassCallback(
        [TextState, &PIC](StringRef PassID, Any IR, const PreservedAnalyses &) {
          TextState->afterPass(PassID, IR, PIC);
        });
  }

  StringRef JSONPath = getIRTrackerJSONOutputPath();
  if (!JSONPath.empty()) {
    auto JSONState = std::make_shared<IRTrackerJSONState>(JSONPath);
    PIC.registerBeforeNonSkippedPassCallback([JSONState](StringRef PassID,
                                                         Any IR) {
      JSONState->beforePass(PassID, IR);
    });
    PIC.registerAfterPassCallback(
        [JSONState, &PIC](StringRef PassID, Any IR, const PreservedAnalyses &) {
          JSONState->afterPass(PassID, IR, PIC);
        });
  }

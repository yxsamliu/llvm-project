...
data_type InstrProfLookupTrait::ReadData(StringRef K, const unsigned char *D,
  ...
  if (N % sizeof(uint64_t)) {
    fprintf(stderr, "DEBUG: ReadData failed for %s: total data size %lu is not a multiple of 8\n", K.data(), (unsigned long)N);
    return data_type();
  }
  ...
  while (D < End) {
    ...
    if (D + sizeof(uint64_t) > End) {
      fprintf(stderr, "DEBUG: ReadData failed for %s: not enough data for hash. Offset: %ld, End: %ld\n", K.data(), (long)(D - OrigD), (long)N);
      return data_type();
    }
    ...
    if (GET_VERSION(FormatVersion) != IndexedInstrProf::ProfVersion::Version1) {
      if (D + sizeof(uint64_t) > End) {
        fprintf(stderr, "DEBUG: ReadData failed for %s: not enough data for CountsSize. Offset: %ld, End: %ld\n", K.data(), (long)(D - OrigD), (long)N);
        return data_type();
      }
    ...
    if (D + CountsSize * sizeof(uint64_t) > End) {
      fprintf(stderr, "DEBUG: ReadData failed for %s: not enough data for counters. Offset: %ld, End: %ld, CountsSize: %lu\n", K.data(), (long)(D - OrigD), (long)N, (unsigned long)CountsSize);
      return data_type();
    }
    ...
    if (GET_VERSION(FormatVersion) > IndexedInstrProf::ProfVersion::Version10) {
      ...
      if (D + sizeof(uint64_t) > End) {
        fprintf(stderr, "DEBUG: ReadData failed for %s: not enough data for BitmapBytes size. Offset: %ld, End: %ld\n", K.data(), (long)(D - OrigD), (long)N);
        return data_type();
      }
      ...
      if (D + PaddedBitmapBytesSize > End) {
        fprintf(stderr, "DEBUG: ReadData failed for %s: not enough data for bitmap bytes. Offset: %ld, End: %ld, BitmapBytes: %lu, PaddedSize: %lu\n", K.data(), (long)(D - OrigD), (long)N, (unsigned long)BitmapBytes, (unsigned long)PaddedBitmapBytesSize);
        return data_type();
      }
    ...
    if (GET_VERSION(FormatVersion) > IndexedInstrProf::ProfVersion::Version2 &&
      ...
      fprintf(stderr, "DEBUG: ReadData failed for %s: readValueProfilingData failed. Offset: %ld, End: %ld\n", K.data(), (long)(D - OrigD), (long)N);
      DataBuffer.clear();
      return data_type();
    }
  }

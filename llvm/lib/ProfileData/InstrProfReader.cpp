...
Error RawInstrProfReader<IntPtrT>::readNextRecord(NamedInstrProfRecord &Record) {
  ...
  if (NumWaveCounters > Record.Counts.size() ||
      (NumWaveCounters && NumWaveCounters == Record.Counts.size()))
    return error(instrprof_error::malformed, "invalid number of wave counters");
  Record.WaveCounts.assign(Record.Counts.end() - NumWaveCounters,
                           Record.Counts.end());
  Record.Counts.resize(Record.Counts.size() - NumWaveCounters);

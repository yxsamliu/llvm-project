  ...
  if (isSGPR) {
    ...
    case AMDGPU::V_SUB_F32_e64: {
      ...
      case AMDGPU::V_SUB_F32_e64: {
        ...
        Register ActiveLanesVreg = MRI.createVirtualRegister(TRI->getAllocatableClass(VregRC));
        Register DstVreg = MRI.createVirtualRegister(TRI->getAllocatableClass(VregRC));
...
static MachineBasicBlock *emitVLoadStoreIdx(MachineInstr &MI,
  ...
  auto EmitIdxSAddrSubDword = [&](MachineInstr &MI, unsigned BitWidth) {
    MachineOperand &SAddrOp = *TII->getNamedOperand(MI, AMDGPU::OpName::saddr);
    int Offset = TII->getNamedOperand(MI, AMDGPU::OpName::offset)->getImm();
    ...
    SAddrOp.isReg() ? Add.addReg(SAddrOp.getReg())
                    : Add.addFrameIndex(SAddrOp.getIndex());
    Add.addImm(Offset);
    Add.setOperandDead(3); // Dead scc

    Register BitOffset;
    if (BitWidth <= 16) {
      Register ByteOffset =
          MRI.createVirtualRegister(&AMDGPU::SReg_32_XM0_XEXECRegClass);
      BuildMI(MBB, MI, DL, TII->get(AMDGPU::S_AND_B32), ByteOffset)
          .addReg(AddReg)
          .addImm(3)
          .setOperandDead(3); // Dead scc
      BitOffset = MRI.createVirtualRegister(&AMDGPU::SReg_32_XM0_XEXECRegClass);
      BuildMI(MBB, MI, DL, TII->get(AMDGPU::S_LSHL_B32), BitOffset)
          .addReg(ByteOffset)
          .addImm(3)
          .setOperandDead(3); // Dead scc
    }
    ...
    // index should be in the unit of dword
    auto Shift = BuildMI(MBB, MI, DL, TII->get(AMDGPU::S_LSHR_B32), IdxReg);
    if (Offset >= 0 && Offset < (int)ST.getAddressableNumVGPRs(
                                    MFI->getDynamicVGPRBlockSize()) *
                                    4) {
      SAddrOp.isReg() ? Shift.addReg(SAddrOp.getReg())
                      : Shift.addFrameIndex(SAddrOp.getIndex());
      Offset = Offset >> 2;
    } else {
      Shift.addReg(AddReg);
    ...
    Shift.addImm(2u);
    Shift.setOperandDead(3); // Dead scc
    return std::tuple(IdxReg, Offset, BitOffset);
  };
  ...
  auto selectSubDwordSaddr = [&](MachineInstr &MI, unsigned NewOpcode,
    ...
    auto NewMI = BuildMI(MBB, MI, DL, TII->get(NewOpcode))
                     .addReg(Data, getDefRegState(IsLoad))
                     .addReg(VirtualIDX)
                     .addImm(Offset)
                     .addImm(BitWidth)
                     .addReg(BitOffset);
  ...
  auto selectSubDwordWaterfall = [&](MachineInstr &MI, unsigned NewOpcode,
    ...
    Register ByteOffset =
        MRI.createVirtualRegister(&AMDGPU::SReg_32_XM0_XEXECRegClass);
    BuildMI(*LoopBB, InsPt, DL, TII->get(AMDGPU::S_AND_B32), ByteOffset)
        .addReg(AddReg)
        .addImm(3)
        .setOperandDead(3); // Dead scc
    ...
    Register BitOffset =
        MRI.createVirtualRegister(&AMDGPU::SReg_32_XM0_XEXECRegClass);
    BuildMI(*LoopBB, InsPt, DL, TII->get(AMDGPU::S_LSHL_B32), BitOffset)
        .addReg(ByteOffset)
        .addImm(3)
        .setOperandDead(3); // Dead scc
...
SITargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI_,
  ...
  if (Constraint.size() == 1) {
    ...
    default:
      RC = TRI->getAllocatableClass(
          TRI->getVectorSuperClassForBitWidth(BitWidth));
      if (!RC)
        return std::pair(0U, nullptr);
      break;
    }
  ...
  if (Kind != '\0') {
    if (Kind == 'v') {
      RC = AMDGPU::isGFX13Plus(*Subtarget) ? &AMDGPU::VGPR_32RegClass
                                           : &AMDGPU::VGPR_32_Lo256RegClass;
    } else if (Kind == 's') {
      RC = &AMDGPU::SGPR_32RegClass;
    } else if (Kind == 'a') {
    ...
    if (RC) {
      if (NumRegs > 1) {
        ...
        if (SIRegisterInfo::isVGPRClass(RC))
          RC = TRI->getAllocatableClass(TRI->getVGPRClassForBitWidth(Width));

...
static bool areArgUsersValidForSplit(Argument &Arg,
...
static void collectLeafFields(Type *Ty, const DataLayout &DL, uint64_t BaseOffset,
...
static Function *trySplitKernelArguments(Function &F, const GCNSubtarget &ST) {
  ...
  for (Argument &Arg : F.args()) {
    ...
    bool HasValidUsers = Arg.use_empty() ||
                         areArgUsersValidForSplit(Arg, Loads, GEPs);
    if (!HasValidUsers) {
      HandlePassthroughArg(Arg);
      continue;
    }
    ...
    ArgToFieldsMap[&Arg] = std::move(Fields);
    ++OriginalArgIndex;
  }

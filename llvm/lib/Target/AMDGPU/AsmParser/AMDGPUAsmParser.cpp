...
public:
  ...
  bool isVSrc_v4f16() const {
    return isRegOrInlineNoMods(AMDGPU::VS_64RegClassID, MVT::f16) ||
           isLiteralImm(MVT::f16);
  }

  bool isVSrc_v4bf16() const {
    return isRegOrInlineNoMods(AMDGPU::VS_64RegClassID, MVT::bf16) ||
           isLiteralImm(MVT::bf16);
  }
  ...
  bool isVISrc_2048_b32() const {
    return isRegOrInlineNoMods(AMDGPU::Pseudo_VGPR_2048RegClassID, MVT::i32);
  }
...
#include "AMDGPUGenAsmMatcher.inc"
  ...
  bool isGFX1250Only() const { return AMDGPU::isGFX1250Only(getSTI()); }
...
bool AMDGPUAsmParser::isInlineConstant(const MCInst &Inst,
  ...
  case 2: {
    ...
    if (OperandType == AMDGPU::OPERAND_REG_INLINE_C_V2FP16 ||
        ...
        OperandType == AMDGPU::OPERAND_REG_IMM_V4FP16)
    ...
    if (OperandType == AMDGPU::OPERAND_REG_INLINE_C_V2BF16 ||
        ...
        OperandType == AMDGPU::OPERAND_REG_IMM_V4BF16)
...
bool AMDGPUAsmParser::validateMIMGDataSize(const MCInst &Inst, SMLoc IDLoc) {
  ...
  unsigned VDataSize =
      AMDGPU::getRegOperandSize(&getSTI(), &MII, Desc, VDataIdx);
...
bool AMDGPUAsmParser::validateMIMGAddrSize(const MCInst &Inst, SMLoc IDLoc) {
  ...
  unsigned ActualAddrSize =
      IsNSA ? SrsrcIdx - VAddr0Idx
            : AMDGPU::getRegOperandSize(&getSTI(), &MII, Desc, VAddr0Idx) / 4;
  ...
  if (IsNSA) {
    if (hasPartialNSAEncoding() &&
      ...
      unsigned VAddrLastSize =
          AMDGPU::getRegOperandSize(&getSTI(), &MII, Desc, VAddrLastIdx) / 4;
  ...
  for (const auto &Op : Operands) {
    ...
    if (AMDGPU::isVGPR(Reg, MRI)) {
      ...
      if (RegIdx >= 0x100 ||
        ...
        Error(Op->getStartLoc(), "register index is out of range");
        return false;
      }
    }
  }

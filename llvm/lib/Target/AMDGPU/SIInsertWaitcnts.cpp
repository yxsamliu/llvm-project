...
#define AMDGPU_DECLARE_WAIT_EVENTS(DECL)                                       \
  ...
  DECL(EXP_VGPR_SEND_NEXT)       /* write VGPR dest in next neighbor SIMD */   \
  DECL(EXP_VGPR_SEND_PREV)       /* write VGPR dest in prev neighbor SIMD */   \
...
public:
  ...
  bool trySimplify(MachineInstr &Inst) const {
    unsigned Opcode = Inst.getOpcode();
    return Opcode != SIInstrInfo::getNonSoftWaitcntOpcode(Opcode) && !OptNone;
  }
...
class WaitcntGeneratorPreGFX12 final : public WaitcntGenerator {
  static constexpr const WaitEventSet
      WaitEventMaskForInstPreGFX12[NUM_INST_CNTS] = {
          WaitEventSet(
              {VMEM_ACCESS, VMEM_SAMPLER_READ_ACCESS, 
...
protected:
  ...
  static constexpr const WaitEventSet
      WaitEventMaskForInstGFX12Plus[NUM_INST_CNTS] = {
          WaitEventSet({VMEM_ACCESS, GLOBAL_INV_ACCESS, VMEM_IMAGE_READ_ACCESS, VMEM_FORMAT_READ_ACCESS}),
          WaitEventSet({LDS_ACCESS, GDS_ACCESS}),
          WaitEventSet({EXP_GPR_LOCK, GDS_GPR_LOCK, VMW_GPR_LOCK,
                        EXP_PARAM_ACCESS, EXP_POS_ACCESS, EXP_LDS_ACCESS,
                        EXP_VGPR_SEND_PREV, EXP_VGPR_SEND_NEXT}),
          WaitEventSet({VMEM_WRITE_ACCESS, SCRATCH_WRITE_ACCESS, IMAGE_WRITE_ACCESS,
                        RTS_WRITE_ACCESS}),
...
public:
  ...
  WaitcntGeneratorGFX12Plus(const MachineFunction &MF,
                            const AMDGPU::HardwareLimits *Limits,
                            bool IsExpertMode)
      : WaitcntGenerator(MF, IsExpertMode, Limits), IsExpertMode(IsExpertMode) {}
...
// For debug builds, get the debug counter info and adjust if need be
  ...
  // Return the appropriate EXP_VGPR_SEND_* type for Inst,
  // which must be a VNBR instruction.
  WaitEventType getVnbrWaitEventType(const MachineInstr &Inst) const {
    switch (Inst.getOpcode()) {
    case AMDGPU::V_SEND_VGPR_NEXT_B32:
    case AMDGPU::V_SEND_VGPR_NEXT_B32_no_write:
      return EXP_VGPR_SEND_NEXT;
    case AMDGPU::V_SEND_VGPR_PREV_B32:
    case AMDGPU::V_SEND_VGPR_PREV_B32_no_write:
      return EXP_VGPR_SEND_PREV;
    }
    llvm_unreachable("Unhandled VNBR opcode/event.");
  }
...
public:
  ...
  // If Waits contains only one value that is not ~0u, that value
  // is returned, otherwise ~0u is returned.
  unsigned getUnmixedWait(std::initializer_list<unsigned> Waits) const {
    unsigned SingleWait = ~0u;

    for (auto &W : Waits) {
      if (W != ~0u) {
        // Return if a wait has already been seen, the set contains
        // more than one wait.
        if (SingleWait != ~0u)
          return ~0u;
        SingleWait = W;
...
void WaitcntBrackets::updateByEvent(WaitEventType E, MachineInstr &Inst) {
  ...
  if (T == EXP_CNT) {
    // For VNBR instructions, the programmer is responsible for synchronizing
    // vdst_refl, and vsrc is guaranteed to be synchronized.
    if (TII->isVNBR(Inst))
      return;
...
bool WaitcntGeneratorPreGFX12::applyPreexistingWaitcnt(
  ...
  for (auto &II :
    ...
    bool TrySimplify = trySimplify(II);
...
bool SIInsertWaitcnts::generateWaitcntInstBefore(
  ...
  default: {
    ...
    if (MI.isCall()) {
      ...
      if (NeedCheck) {
        Interval =
            ScoreBrackets.getRegInterval(&MI, MI.getOperand(0));
        ScoreBrackets.determineWait(VA_VDST, Interval, Wait);
  ...
  if (TII->isVALU(MI) &&
      (!MI.isBundled() || !isa<AMDGPUMI::VStoreIdxInst>(MI))) {
    Wait.VaVdst = ~0u;
    // In non-expert mode VALU instructions wait for VM_VSRC to be 0
    if (!IsExpertMode) {
      Wait.VmVsrc = ~0u;
      ScoreBrackets.applyWaitcnt(VM_VSRC, 0);
    }
  }
...
bool SIInsertWaitcnts::generateWaitcnt(AMDGPU::Waitcnt Wait,
  ...
  if (Wait.VaVdst != ~0u && It != Block.instr_end() && TII->isVNBR(*It)) {
    MachineOperand *WaitVaVdst =
        TII->getNamedOperand(*It, AMDGPU::OpName::wait_va_dst);
    if (Wait.VaVdst < WaitVaVdst->getImm()) {
      WaitVaVdst->setImm(Wait.VaVdst);
      Modified = true;
    }
    Wait.VaVdst = ~0u;
...
void SIInsertWaitcnts::updateEventWaitcntAfter(MachineInstr &Inst,
  ...
  if (TII->isDS(Inst) && TII->usesLGKM_CNT(Inst)) {
    ...
    if (Inst.getOpcode() == AMDGPU::RTS_READ_RESULT_ALL_STOP ||
      ...
      ScoreBrackets->updateByEvent(VMEM_BVH_READ_ACCESS, Inst);
    }
    int FlatASCount = 0;
  ...
  } else if (TII->isVNBR(Inst) &&
             AMDGPU::hasNamedOperand(Inst.getOpcode(), AMDGPU::OpName::vdst)) {
    int64_t Imm =
        TII->getNamedOperand(Inst, AMDGPU::OpName::wait_va_dst)->getImm();
    if (Imm < 0xF)
      ScoreBrackets->applyWaitcnt(VA_VDST, Imm);
    ScoreBrackets->updateByEvent(getVnbrWaitEventType(Inst), Inst);
...
static bool isWaitInstr(MachineInstr &Inst) {
  ...
  return Opcode == AMDGPU::S_WAITCNT ||
         ...
         // S_WAIT_DEPCTR is only considered a wait instr when inserted by the
         // memory legalizer as a soft waitcnt, since it's special cased
         // otherwise.
         Inst.getOpcode() == AMDGPU::S_WAIT_DEPCTR_soft ||
...
SIInsertWaitcnts::getPreheaderFlushFlags(MachineLoop *ML,
  ...
  DenseSet<Register> VgprDefVMEM;
  DenseSet<Register> VgprDefDS;
    ...
    for (MachineInstr &MI : *MBB) {
      ...
      for (const MachineOperand &Op : MI.all_uses()) {
        ...
        for (int RegNo = Interval.first; RegNo < Interval.second; ++RegNo) {
          ...
          bool HasPendingVMEM =
              Brackets.getRegScore(RegNo, LOAD_CNT) >
                  Brackets.getScoreLB(LOAD_CNT) ||
              Brackets.getRegScore(RegNo, SAMPLE_CNT) >
                  Brackets.getScoreLB(SAMPLE_CNT) ||
              Brackets.getRegScore(RegNo, BVH_CNT) > Brackets.getScoreLB(BVH_CNT);
          if (HasPendingVMEM)
          ...
          if (!HasPendingVMEM &&
              Brackets.getRegScore(RegNo, DS_CNT) > Brackets.getScoreLB(DS_CNT))
